...
export interface UseIncrementalFunnelResult<TStepId extends FunnelStepId = FunnelStepId> {
  currentStepId: TStepId;
  steps: readonly FunnelStep<TStepId>[];
...
export function useIncrementalFunnel<TStepId extends FunnelStepId>(
  ...
  const initialState = useMemo(() => createFunnel(steps, initialStepId), [steps, initialStepId]);
